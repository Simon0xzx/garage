...
@click.option('--num_steps_prior', default=800)
@click.option('--num_extra_rl_steps_posterior', default=800)
...
@click.option('--prefix', default='tcl_pearl_v2')
@wrap_experiment
def tcl_pearl_v2_metaworld_ml1(
    ctxt=None,
    seed=0,
    num_epochs=100,
    num_train_tasks=50,
    ...
    """
    set_seed(seed)
    encoder_hidden_sizes = tuple([encoder_hidden_size] * 3)
    ...
    augmented_env = TCLPEARLV2.augment_env_spec(env[0](), latent_size)
    ...
    curl = CURL(latent_size, 1, distance='mahalanobis')
    contrastive_loss = cross_entropy_loss_cls()
    tcl_pearl = TCLPEARLV2(
        env=env,
