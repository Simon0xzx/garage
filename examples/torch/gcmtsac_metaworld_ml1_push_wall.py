...
def gcmtsac_metaworld_ml1_push_wall(ctxt=None,
                                      seed=1,
                                      net_size=400,
                                      goal_dim=3,
                                      max_path_length=150,
    ...
    for i in range(num_train_tasks):
        train_env = GarageEnv(normalize(mwb.ML1.get_train_tasks('push-wall-v1'),
                      normalize_reward=True))
        test_env = pickle.loads(pickle.dumps(train_env))
        env_names.append('push-wall-{}'.format(i))
...
if __name__ == '__main__':
    gcmtsac_metaworld_ml1_push_wall()
