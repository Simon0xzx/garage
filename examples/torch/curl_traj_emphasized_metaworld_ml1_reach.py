...
@click.option('--num_test_tasks', default=20)
...
@wrap_experiment
def curl_traj_emphasized_metaworld_ml1_reach(ctxt=None,
                             seed=1,
                             num_epochs=1000,
                             num_train_tasks=50,
                             num_test_tasks=10,
    ...
    env_sampler = SetTaskSampler(lambda: GarageEnv(
        normalize(mwb.ML1.get_train_tasks('reach-v1'))))
    env = env_sampler.sample(num_train_tasks)

    test_env_sampler = SetTaskSampler(lambda: GarageEnv(
        normalize(mwb.ML1.get_test_tasks('reach-v1'))))
...
curl_traj_emphasized_metaworld_ml1_reach()
