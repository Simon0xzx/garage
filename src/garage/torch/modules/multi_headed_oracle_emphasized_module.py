...
class MultiHeadedOracleEmphasizedModule(nn.Module):
    ...
    def __init__(self,
        ...
        if layer_normalization:
            context_layers.add_module('layer_normalization',
                                     nn.LayerNorm(prev_size))
        linear_layer = nn.Linear(latent_sizes, 200)
        hidden_w_init(linear_layer.weight)
        hidden_b_init(linear_layer.bias)
        context_layers.add_module('linear', linear_layer)

        if hidden_nonlinearity:
            context_layers.add_module('non_linearity',
                                     _NonLinearity(hidden_nonlinearity))
