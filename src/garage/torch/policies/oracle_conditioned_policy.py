...
class OracleConditionedPolicy(nn.Module):
    ...
    def reset_belief(self, num_tasks=1):
        ...
        self.z_means = torch.zeros(num_tasks, self._latent_dim).to(global_device())
        self.z_vars = torch.zeros(num_tasks, self._latent_dim).to(global_device())
        # sample a new z from the prior
        self.sample_from_belief(self.z_means)
        # reset the context collected so far
        self._context = None

    def sample_from_belief(self, goal):
        self.z_means = torch.tensor(goal, dtype=torch.float32 ,device=global_device())
        self.z = self.z_means
