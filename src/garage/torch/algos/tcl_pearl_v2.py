...
class TCLPEARLV2(MetaRLAlgorithm):
    r"""A contrastive PEARL model based on https://arxiv.org/abs/1903.08254.
    ...
    Args:
        ...
        qf1 (torch.nn.Module): Q-function.
        qf2 (torch.nn.Module): Q-function.
    ...
    def __init__(
            ...
            use_contrastive_classification,
        ...
        self._qf1 = qf1
        self._qf2 = qf2
        self._target_qf1 = copy.deepcopy(qf1)
        self._target_qf2 = copy.deepcopy(qf2)
        ...
        self._contrastive_module = contrastive_module
        self._use_contrastive_classification = use_contrastive_classification
        self._contrastive_loss_module = contrastive_loss_cls()
        self.contrastive_optimizer = optimizer_class(
            self._contrastive_module.online_parameters(),
            lr=contrastive_lr,
        )
        self._contrastive_accuracies = []
    ...
    def train(self, trainer):
        ...
        for _ in trainer.step_epochs():
            ...
            self._z_means = np.concatenate(self._z_means, axis=0)
            self._z_vars = np.concatenate(self._z_vars, axis=0)
            z_mean_mean = np.mean(np.mean(self._z_means, axis=0))
            z_mean_std = np.mean(np.std(self._z_means, axis=0))
            z_sig_mean = np.mean(np.mean(self._z_vars, axis=0)) 
            z_sig_std = np.mean(np.std(self._z_vars, axis=0)) 
            contrastive_accuracy_mean = np.mean(self._contrastive_accuracies)
            contrastive_loss_mean = np.mean(self._contrastive_losses)
            tabular.record('Z_eval_mean/mean', z_mean_mean)
            tabular.record('Z_eval_mean/std', z_mean_std)
            tabular.record('Z_eval_variance/mean', z_sig_mean)
            tabular.record('Z_eval_variance/std', z_sig_std)
            tabular.record('Z_eval_variance/std', z_sig_std)
            tabular.record('ContrastiveAccuracy/mean', contrastive_accuracy_mean)
            tabular.record('ContrastiveLoss/mean', contrastive_loss_mean)
    ...
    def _optimize_policy(self, indices):
        ...
        with torch.no_grad():
            alpha = self._get_log_alpha(indices).exp().view(-1, 1)
        ...
        new_next_actions, policy_mean, policy_log_std, new_log_pi = policy_outputs[:4]
        ...
        target_q_values = torch.min(
            self._target_qf1(torch.cat([next_obs, new_next_actions], dim=1), task_z),
            self._target_qf2(
                torch.cat([next_obs, new_next_actions], dim=1), task_z)) - (alpha * new_log_pi)
        ...
        with torch.no_grad():
            q_target = rewards_flat * self._reward_scale + (
                1. - terms_flat) * self._discount * target_q_values
        qf1_loss = F.mse_loss(q1_pred.flatten(), q_target.flatten())
        qf2_loss = F.mse_loss(q2_pred.flatten(), q_target.flatten())
        ...
        policy_outputs, _ = self._policy(obs.view(t, b, -1), context)
        ...
        if self._use_automatic_entropy_tuning:
            alpha_loss = (-(self._get_log_alpha(indices).view(-1, 1)) *
    ...
    def _optimize_contrastive_loss(self, indices):
        ...
        # clear context and reset belief of policy
        self._policy.reset_belief(num_tasks=num_tasks)
        self._target_policy.reset_belief(num_tasks=num_tasks)

        obs, actions, rewards, next_obs, terms = self._sample_data(indices)
        query_context, key_context = self._sample_contrastive_pair(indices)

        # contrastive optimization
        _, _ = self._policy(next_obs, query_context)
        query_task_z = torch.cat([self._policy.z_means, 
                                self._policy.z_vars], dim=-1)
        with torch.no_grad():
            _, _ = self._target_policy(next_obs, key_context)
            key_task_z = torch.cat([self._target_policy.z_means, 
                                    self._target_policy.z_vars], dim=-1)
        ...
        if self._use_contrastive_classification:
            logits = self._contrastive_module.compute_logits(query_task_z, key_task_z)
            labels = torch.arange(logits.shape[0]).long().to(global_device())
            contrastive_loss = self._contrastive_loss_module(logits, labels)
            contrastive_accuracy = torch.sum(torch.max(logits, 1)[1] == labels).float() / num_tasks
            self._contrastive_accuracies.append(contrastive_accuracy.item())
        self._contrastive_losses.append(contrastive_loss.item())
    ...
    # assumes that all paths have same length
    def _sample_paths(self, indices, replay_buffer=None):
        # make method work given a single task index
        if not hasattr(indices, '__iter__'):
            indices = [indices]
        if replay_buffer is None:
            replay_buffer = self._context_replay_buffers
        initialized = False
        for idx in indices:
            batch = replay_buffer[idx].sample_path()
            o = batch['observations']
            a = batch['actions']
            r = batch['rewards']
            context = np.hstack((np.hstack((o, a)), r))
    ...
    def _sample_contrastive_pair(self, indices):
        # shape (T, N, D)
        context_paths = self._sample_paths(indices, replay_buffer=self._context_replay_buffers)
        path_length = context_paths.size()[1]
        query_context = self._contrastive_augmentation(context_paths, ordered=False)
        key_context = self._contrastive_augmentation(context_paths, ordered=False)
        return query_context, key_context
    
    def _contrastive_augmentation(self, context, ordered=False):
        path_length = context.size()[1]
        if ordered:
            start = np.random.randint(self._contrastive_start, path_length - self._embedding_batch_size)
            aug = context[:, start: start + self._embedding_batch_size, :]
        else:
            aug = context[:, np.random.choice(path_length, size=self._embedding_batch_size),:]
        return aug

    def _update_target_network(self):
        """Update parameters in the target network."""
        online_nets = [self._qf1, self._qf2, self._policy]
        target_nets = [self._target_qf1, self._target_qf2, self._target_policy]
        for online_net, target_net in zip(online_nets, target_nets):
            for target_param, param in zip(target_net.parameters(),
                                        online_net.parameters()):
                target_param.data.copy_(target_param.data *
                                        (1.0 - self._soft_target_tau) +
                                        param.data * self._soft_target_tau)
        self._contrastive_module.update_target_networks(self._soft_target_tau)
    ...
    def networks(self):
        ...
        return self._policy.networks + [self._policy] + [
            self._qf1, self._qf2, self._target_qf1, self._target_qf2, self._contrastive_module
        ] + self._target_policy.networks + [self._target_policy]
    ...
    def adapt_policy(self, exploration_policy, exploration_episodes):
        ...
        self._z_means.append(self._policy.z_means.detach().cpu().numpy())
        self._z_vars.append(self._policy.z_vars.detach().cpu().numpy())
    ...
    def to(self, device=None):
        ...
        if not self._use_automatic_entropy_tuning:
            self._log_alpha = torch.Tensor([self._fixed_alpha
                                            ] * self._num_train_tasks).log().to(device)
        else:
            self._log_alpha = torch.Tensor([self._initial_log_entropy
                                            ] * self._num_train_tasks).to(device).requires_grad_()
...
class TCLPEARLV2Worker(DefaultWorker):
    """A worker class used in sampling for TCLPEARL.
...
class BYOL(nn.Module):
    def __init__(self, latent_dim, projection_dim, hidden_sizes):
        super(BYOL, self).__init__()
        self._latent_dim = latent_dim
        self._projection_dim = projection_dim

        self.query_projection_head = MLPEncoder(latent_dim, projection_dim, hidden_sizes)
        self.key_projection_head = MLPEncoder(latent_dim, projection_dim, hidden_sizes)
        self.predictor = MLPEncoder(projection_dim, projection_dim, hidden_sizes)

    def compute_half_symmetric_error(self, query, key):
        query_proj = self.query_projection_head(query)
        prediction = self.predictor(query_proj)
        prediction_normalized = prediction / torch.norm(prediction)
        with torch.no_grad():
            key_proj = self.key_projection_head(key)
            key_proj_normalized = key_proj / torch.norm(key_proj)

        return prediction_normalized, key_proj_normalized
        
    def compute_error(self, query, key):
        return self.compute_half_symmetric_error(query, key)

    def online_parameters(self):
        return list(self.query_projection_head.parameters()) + list(self.predictor.parameters())
    
    def target_parameters(self):
        return self.key_projection_head.parameters()
    
    def update_target_networks(self, m):
        for target_param, param in zip(self.key_projection_head.parameters(),
...
class CURL(nn.Module):
    def __init__(self, latent_dim, temperature, distance='mahalanobis'):
        super(CURL, self).__init__()
        self._latent_dim = latent_dim
        self.distance = distance
        self.W_mean = nn.Parameter(torch.rand(latent_dim, latent_dim))
        self.W_var = nn.Parameter(torch.rand(latent_dim, latent_dim))
        self.temperature = temperature

    def online_parameters(self):
        return [self.W_mean, self.W_var]
    
    def target_parameters(self):
        return []
    
    def compute_logits(self, z_query, z_key):
        """
        ...
        """
        query_mean = z_query[:, :self._latent_dim]
        key_mean = z_key[:, :self._latent_dim]
        query_var = z_query[:, self._latent_dim:]
        key_var = z_key[:, self._latent_dim:]
        B, D = query_mean.size()

        if self.distance == 'bilinear':
            Wz = torch.matmul(self.W_mean, key_mean.T)  # (z_dim,B)
            logits = torch.matmul(query_mean, Wz)  # (B,B)
        elif self.distance == 'mahalanobis':
            #query_mean = torch.matmul(query_mean, self.W_mean)
            #query_var = torch.matmul(query_var, self.W_var)

            query_mean = query_mean.unsqueeze(1).expand(B, B, D)
            key_mean = key_mean.unsqueeze(0).expand(B, B, D)
            query_var = query_var.unsqueeze(1).expand(B, B, D)
            key_var = key_var.unsqueeze(0).expand(B, B, D)

            mean_dist = torch.pow(query_mean - key_mean, 2)
            var_dist = (query_var + key_var) / 2.
            logits = torch.div(mean_dist, 1e-3 + var_dist)
            logits = -logits.sum(2)
