...
class MLSAC(SAC):
    """A MLSAC Model in Torch.

    This MLSAC implementation uses is the same as SAC except for a small change
    ...
    The alphas are accessed by using a the task embedding that is
    ...
    Args:
        ...
        embeddings (list[np.ndarray]): List of task embeddings.
    ...
    def __init__(
        ...
        *,
        ...
        self._embedding_size = len(embeddings[0])
        self._embeddings_dict = {emb.astype(np.float32).tobytes():i for i, emb in enumerate(embeddings)}
    ...
    def _get_log_alpha(self, samples_data):
        ...
        """
        obs = samples_data['observation'].detach().cpu().numpy().astype(np.float32)
        embeddings = obs[:,-self._embedding_size:]
        log_alpha = self._log_alpha
        # if self._use_single_alpha:
        #     task_indices = np.zeros(len(obs))
        # else:
        #     task_indices = [self._embeddings_dict[ob.tobytes()] for ob in obs]
        task_indices = [self._embeddings_dict[emb.tobytes()] for emb in embeddings]
        one_hots = torch.eye(self._num_tasks).to(global_device())
        one_hots = one_hots[task_indices]
