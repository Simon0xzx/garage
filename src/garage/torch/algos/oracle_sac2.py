...
class ORACLESAC2(MetaRLAlgorithm):
...
class ORACLESAC2Worker(DefaultWorker):
    ...
    def rollout(self):
        ...
        """
        self.cur_goal = self.env._env._current_env.unwrapped.goal[None]
        print(self.cur_goal)
        self.agent.sample_from_belief(self.cur_goal)
        self.start_episode()
        while not self.step_episode():
            pass
        epoch_result = self.collect_episode()
        epoch_result.env_infos['goal'] = np.repeat(self.cur_goal, epoch_result.lengths[0], axis=0)
        return epoch_result
