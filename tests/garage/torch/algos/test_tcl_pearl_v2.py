"""This script is a test that fails when TCLPEARLV2 performance is too low."""
import pickle
...
from garage.torch.algos import TCLPEARLV2
from garage.torch.algos.tcl_pearl_v2 import TCLPEARLV2Worker, CURL, cross_entropy_loss_cls
...
@pytest.mark.mujoco
class TestTCLPEARLV2:
    """Test class for TCLPEARLV2."""

    # @pytest.mark.skip
    @pytest.mark.large
    def test_pearl_ml1_push(self):
        """Test TCLPEARLV2 with ML1 Push environment."""
        params = dict(seed=1,
                      num_epochs=1,
                      num_train_tasks=5,
                      latent_size=7,
                      ...
                      use_contrastive_classification=True,
                      )
        ...
        augmented_env = TCLPEARLV2.augment_env_spec(env[0](), params['latent_size'])
        ...
        curl = CURL(params['latent_size'], 1, distance='mahalanobis')
        contrastive_loss = cross_entropy_loss_cls()
        ...
        pearl = TCLPEARLV2(
            ...
            qf2=qf2,
            sampler=sampler,
            contrastive_module=curl,
            contrastive_loss_cls=contrastive_loss,
            use_contrastive_classification=params['use_contrastive_classification'],
        ...
        set_gpu_mode(params['use_gpu'], gpu_id=5)
        if params['use_gpu']:
            pearl.to()
    ...
    def test_pickling(self):
        ...
        augmented_env = TCLPEARLV2.augment_env_spec(env[0](), 5)
        qf1 = ContinuousMLPQFunction(
            env_spec=augmented_env,
            hidden_sizes=[net_size, net_size, net_size])
        ...
        curl = CURL(5, 1, distance='mahalanobis')
        contrastive_loss = cross_entropy_loss_cls()

        pearl = TCLPEARLV2(env=env,
                      inner_policy=inner_policy,
                      qf1=qf1,
                      qf2=qf2,
                      sampler=None,
